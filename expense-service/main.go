...
func main() {
   ...
   // r.HandleFunc("/expenses", getAllExpenses).Methods("GET")
    ...
    r.HandleFunc("/expense-service", getAllExpenses).Methods("GET")
    r.HandleFunc("/expense-service/add", createExpense).Methods("POST")
    r.HandleFunc("/expense-service/{userId}", getExpensesByUser).Methods("GET")

    c := cors.New(cors.Options{
        AllowedOrigins: []string{"http://3.91.202.159"},
